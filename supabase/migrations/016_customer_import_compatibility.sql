-- Live SMARTBILLER compatibility migration.
-- The existing production schema uses nama_bil (not nama_b11) in customer_import_staging.
-- Keep nama_b11 only as a generated compatibility alias for older queries.

alter table public.customer_import_staging
  add column if not exists nama_b11 text generated always as (nama_bil) stored;

create index if not exists idx_customer_import_staging_idpel
  on public.customer_import_staging (idpel);

create index if not exists idx_customer_import_staging_batch
  on public.customer_import_staging (import_batch_id);
...
begin
  ...
  for r in
    select *
    from public.customer_import_staging
  ...
  loop
    ...
    order by case
      when lower(coalesce(p.full_name, '')) = lower(coalesce(r.username, '')) then 0
      else 1
    end
    limit 1;

    -- Do not import an unowned customer. Ownership is required by the live RLS model.
    ...
    if v_code is not null
       and nullif(trim(r.region), '') is not null
       and nullif(trim(r.ulp), '') is not null then
      v_sequence := case v_code
        when 'A' then 1
        when 'B' then 2
        when 'C' then 3
        when 'D' then 4
      ...
      insert into public.bmax_rbms (
        biller_user_id, region, ulp, code, name, sequence_no, active
      )
      values (
        v_profile_id,
        trim(r.region),
      ...
      where b.biller_user_id = v_profile_id
        and b.code = v_code
      limit 1;
    end if;

    v_segment := upper(coalesce(nullif(trim(r.segment), ''), 'PREVENTIF'));
    if v_segment not in ('PREVENTIF','IRISAN','KOREKTIF') then
      v_segment := 'PREVENTIF';
    end if;
    ...
    insert into public.customers (
      ...
      bmax_rbm_id, bmax_biller_id, username, nama_bil, kode_petugas, tiang
    ) values (
      trim(r.idpel),
      nullif(trim(r.meter_number), ''),
      trim(r.name),
      nullif(trim(r.phone), ''),
      nullif(trim(r.address), ''),
      r.latitude,
      r.longitude,
      nullif(trim(r.tariff), ''),
      r.power_va,
      nullif(trim(r.ulp), ''),
      nullif(trim(r.rbm), ''),
